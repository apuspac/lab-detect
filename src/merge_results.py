...
class MergeResults():
    ...
    def merge_results(self, filename, filepath):
        ...
        input_mask_file = open(self.mask_results_location)

        bbox_results = json.load(input_bbox_file)['yolo']
        mask_results = json.load(input_mask_file)['yolact']
...
if __name__ == '__main__':
    # 検出に使用したimgディレクトリの読み込み
    input_folder_path = parse_args(sys.argv)
    # result内フォルダ名を第二引数で入力
    json_result_path_name = sys.argv[2]

    merge_ins = MergeResults(json_result_path_name)
    merge_results = []
